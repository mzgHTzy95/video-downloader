...
class VideoDownloader:
    ...
    def setup_ui(self):
        ...
        main_canvas = tk.Canvas(self.root, highlightthickness=0, bg=self.colors['background'])
        scrollbar = ttk.Scrollbar(self.root, orient="vertical", command=main_canvas.yview)
        scrollable_frame = ttk.Frame(main_canvas)
        
        scrollable_frame.bind(
            "<Configure>",
            lambda e: self._on_frame_configure(main_canvas)
        )
        ...
        url_card.configure(highlightbackground=self.colors['border'], highlightthickness=1)
        ...
        quality_card = tk.Frame(content_frame, bg=self.colors['card'], relief=tk.FLAT, bd=0)
        quality_card.pack(fill=tk.X, pady=(0, 15))
        quality_card.configure(highlightbackground=self.colors['border'], highlightthickness=1)
        ...
        # Download Path Card
        path_card = tk.Frame(content_frame, bg=self.colors['card'], relief=tk.FLAT, bd=0)
        path_card.pack(fill=tk.X, pady=(0, 15))
        path_card.configure(highlightbackground=self.colors['border'], highlightthickness=1)
        ...
        # Progress Card
        progress_card = tk.Frame(content_frame, bg=self.colors['card'], relief=tk.FLAT, bd=0)
        ...
        self.progress_canvas = tk.Canvas(progress_bg, bg="#F0F0F0", height=8, highlightthickness=0)
        self.progress_canvas.pack(fill=tk.BOTH, expand=True)
        self.progress_bar = self.progress_canvas.create_rectangle(0, 0, 0, 8, fill=self.colors['primary'], outline="")
    ...
    def ensure_ffmpeg(self):
        ...
        # Notify user
        try:
            self.status_label.config(text="Downloading FFmpeg (first run)...", fg="#FF9800")
        ...
        try:
            ...
            try:
                self.status_label.config(text="FFmpeg ready", fg=self.colors.get("accent", "#00C853"))
                self.root.update_idletasks()
            except Exception:
                pass
    ...
    def cancel_download_action(self):
        ...
        if self.is_downloading:
            ...
            self.download_btn.config(state=tk.NORMAL, text="⬇ Download Now", bg=self.colors['primary'])
    ...
    def download_video(self):
        ...
        try:
            ...
            ydl_opts = {
                ...
                "postprocessor_args": ["-c:v", "copy", "-c:a", "aac"],  # Preserve quality during merge
            }
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                if is_playlist and "entries" in info:
                    ...
                    for idx, entry in enumerate(entries, 1):
                        ...
                        try:
                            ydl.download([entry.get('url') or entry.get('webpage_url') or url])
                        except Exception as e:
            ...
            self.status_label.config(text="Download completed!", fg=self.colors['accent'])
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if version.parse(latest_version) > version.parse(self.VERSION):
                    ...
                    messagebox.showinfo("No Updates", f"You are using the latest version ({self.VERSION})")
        except Exception as e:
            if not auto:
